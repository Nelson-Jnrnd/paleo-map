...
test('shows a loading indicator while the snapshot loads (FONC-1260)', () => {
  const pending: () => Promise<ReadModel> = () => new Promise<ReadModel>(() => {});
  render(<App loader={pending} />);
  expect(screen.getByRole('status')).toHaveTextContent(/Loading the dated snapshot/i);
});
...
test('shows an error with Retry that recovers (FONC-1310/1330)', async () => {
  ...
  expect(await screen.findByRole('navigation', { name: /timeline/i })).toBeInTheDocument();
});

test('labels a minimal (occurrence-only) profile (FONC-1300)', async () => {
  const api = await fixtureApi();
  render(<TaxonProfile api={api} taxonId="txn:nanotyrannus" onBack={() => {}} />);
