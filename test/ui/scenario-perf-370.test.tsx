...
test('PERF-370: empty result → empty state → reset filters', async () => {
  ...
  expect(within(emptyState).getByText(/No occurrences at this age/i)).toBeInTheDocument();
  ...
  expect(screen.queryByText(/No occurrences at this age/i)).not.toBeInTheDocument();
  expect(within(timeline).getByRole('button', { name: /Maastrichtian/ })).toHaveAttribute(
    'aria-pressed',
    'true',
  );
  // Occurrences are back (a Tyrannosaurus row is visible again).
  expect(screen.getAllByRole('button', { name: /Tyrannosaurus/ }).length).toBeGreaterThan(0);
