...
test("PERF-340: period → filter dinosaurs → occurrence → profile → back to map", async () => {
  ...
  const openProfile = screen.getByRole("button", {
    name: /Open taxon profile/i,
  });
  ...
  const profile = await screen.findByRole("region", {
    name: /Taxon profile: Tyrannosaurus/i,
  });
  expect(
    within(profile).getByRole("heading", { name: "Tyrannosaurus" }),
  ).toBeInTheDocument();
  ...
  const timelineAgain = await screen.findByRole("navigation", {
    name: /timeline/i,
  });
  expect(
    within(timelineAgain).getByRole("button", { name: /Campanian/ }),
