...
export function TaxonProfile({ api, taxonId, onBack }: TaxonProfileProps): ReactElement {
  ...
  return (
    <section className={styles.profile} aria-label={`Taxon profile: ${taxon.scientificName}`}>
      <button type="button" className={styles.back} onClick={onBack}>
        ← Back to map
      </button>
      ...
      <header className={styles.profileHeader}>
        ...
        {isMinimal && <AttentionNote>Occurrence only — minimal profile</AttentionNote>}
      </header>
      ...
      <div className={styles.section}>
        <span className={styles.statLabel}>Occurrences ({occurrences.length})</span>
          ...
          {occurrences.map((o) => {
            ...
            return (
              <li key={o.id} className={styles.occCard}>
                ...
                <span className={styles.occurrenceMeta}>
                  <span>Modern: {o.modernPosition.value?.region ?? NOT_AVAILABLE}</span>
                  {paleo ? (
                    <span>
                      Paleo:{' '}
                      <span className="mono">
      ...
      <div className={`${styles.section} ${styles.interpretativeBlock}`}>
        ...
        <p className={styles.source}>Inferred biology — separated from fossil-derived data.</p>
        ...
        <dl className={styles.fieldGrid}>
          {(profile?.attributes ?? []).map((attr) => (
            <div key={attr.kind} style={{ display: 'contents' }}>
              ...
              <dd className={styles.fieldValue}>{attr.value.value ?? NOT_AVAILABLE}</dd>
            </div>
          ))}
